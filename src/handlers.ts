...
import {addRuleForm, removeRuleForm} from "./main.js";
...
export async function onFormSubmitRemoveRuleHandler(event: FormOnSubmitEvent<any>, context: Context) {
    let currentVersion = new Current();
...
async function rejectSubmit(
    error: string,
    ui: Context["ui"],
    values: { duration: string; name: string; rule: string; startCron: string },
): Promise<void> {
...
export async function onFormSubmitAddRuleHandler(event: FormOnSubmitEvent<any>, context: Context): Promise<void> {
    let currentVersion = new Current();
